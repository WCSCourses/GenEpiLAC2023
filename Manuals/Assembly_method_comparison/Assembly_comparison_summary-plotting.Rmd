...
Paraguay_assembly.stats %>%
  ...
  mutate(method_full=factor(`Student Name`, levels=unique(`Student Name`))) %>%
  ggplot(aes(y=`Student Name`, x=`N50`, fill=`Student Name`)) +
  #geom_quasirandom(width=0.4, alpha=0.75, groupOnX=F) +
  geom_barh(width=0.5, stat='identity') + 
  theme_bw() + theme(legend.position = 'none') + 
  theme.text.size + 
  geom_text(aes((N50+150000), `Student Name`,label=`Student Name`)) +
  labs(y="Student", x="N50", title="N50 for each Student")


ggsave(paste0("Student.by.N50.png"), units='mm', width=120, height=70, device='png', dpi=300)
